...
export type TokenKind =
  | "INDENT"        // significant leading whitespace level
  | "ELEMENT"       // tag name, @define, @slot, @style, @ComponentName, |
  | "ATTR_CHAIN"    // .attr("val") or .boolAttr raw segment
  | "MULTILINE_START" // element ends with ':'
  | "MULTILINE_LINE"  // content line inside a multiline block
  | "COMMENT"       // // ...
  | "BLANK"         // empty line
  | "EOF";
...
export function tokenize(source: string): Token[] {
  ...
  while (lineIdx < rawLines.length) {
    ...
    if (stripped.startsWith("\t")) {
      throw new NMLLexerError(
        `Indentation error on line ${currentLine}: Please use 4 spaces for indentation, not tabs.`,
        { line: currentLine, column: 0 }
      );
    }
    if (leadingSpaces % INDENT_WIDTH !== 0) {
      throw new NMLLexerError(
        `Indentation error on line ${currentLine}: Non-standard indentation. Use 4 spaces per level.`,
        { line: currentLine, column: 0 }
      );
    }
 ...
 */
function isLineMultilineTrigger(content: string): boolean {
  let inQuote = false;
  let quoteChar = "";
  let i = 0;
